...
FrozenTrie.prototype = {
    ...
    lookup: function (word) {
        const norm = word.replace(/æ/g,'{').replace(/ø/g,'|').replace(/å/g,'}')
    ...
    childList: function(str,node) {
        let list = [];
        if (node.final) list.push(str + node.letter.replace('{','æ').replace('|','ø').replace('}','å'));
        let child,j=0;
        str += node.letter.replace('{','æ').replace('|','ø').replace('}','å');
        for (; j < node.getChildCount(); j++) {
            child = node.getChild(j);
            list = list.concat(this.childList(str,child));
        }
        return list;
    }
...
// the lookup function
export const trial = (dict) => {
    var json = JSON.parse(dict);
...
export function trieup(dict, word) {
    var status = "";
    try {
        var json = JSON.parse(dict);
        var ftrie = new FrozenTrie(json.trie, json.directory, json.nodeCount
        ...
        return "Error. Have you encoded the dictionary yet?";
    }
